...
<main id="response-container" class="max-w-[1024px] mx-auto w-full {$chatHistry.length > 0 ? 'mb-[95px]' : ''} h-full">
  <h1 class="text-2xl font-bold py-4 px-1 text-center">Techunter AI Prompt Application</h1>

  {#if $isLoading || (Array.isArray($chatHistry) && $chatHistry.length >= 0)}
    <div class="md:p-4 rounded max-w-[1550px] w-full" id="response">
      <h2 class="text-lg font-semibold text-center">Write your prompt</h2>
    </div>
  {/if}

  <div class="rounded w-full {$chatHistry.length > 0 ? 'md:p-4' : ''}">
    {#if Array.isArray($chatHistry) && $chatHistry.length > 0}
      {#each $chatHistry as value, index (index)}
        <div class="md:max-w-[50%] md:ml-auto max-w-full w-full">
          ...
          <div class="user-response bg-gray-950 text-white text-end w-fit ml-auto px-3 rounded my-2">
            <p>{value.prompt}</p>
          </div>
        </div>
        <div class="md:max-w-[60%] max-w-full w-full text-balance">
          <span class="text-white font-bold">Techunter AI</span>
          {#if !value.response}
          ...
          <div class="bg-gray-950 px-3 rounded {value.response ? 'py-4' : ''} relative">
